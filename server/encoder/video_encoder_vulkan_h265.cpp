...
std::vector<void *> video_encoder_vulkan_h265::setup_slot_info(size_t dpb_size)
{
...
std::vector<uint8_t> video_encoder_vulkan_h265::get_vps_sps_pps()
{
...
void video_encoder_vulkan_h265::send_idr_data()
{
	auto data = get_vps_sps_pps();
	SendData(data, false);
}

void * video_encoder_vulkan_h265::encode_info_next(uint32_t frame_num, size_t slot, std::optional<size_t> ref)
{
	slice_header = {
	        .flags =
	                {
...
vk::ExtensionProperties video_encoder_vulkan_h265::std_header_version()
{
